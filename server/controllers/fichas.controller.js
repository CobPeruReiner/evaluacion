...
const getAllFichas = catchAsync(async (req, res, next) => {
    ...
    const fichas = await Ficha.findAll({
        ...
        where: {
            [Op.and]: [
                literal(`STR_TO_DATE(fecha_monitoreo, '%d/%m/%Y') BETWEEN '${formattedStartDate}' AND '${formattedEndDate}'`)
            ]
        }
    });
            ...
            Sequelize.where(
                Sequelize.fn('YEAR',
                    Sequelize.fn('STR_TO_DATE',
                        Sequelize.col('fecha_llamada'), '%d/%m/%Y')), currentYear)
            ]
        }
    });
...
const getPromedioAnualCalificacion = async (req, res) => {
    ...
    try {
        ...
        const promedio = await Ficha.findOne({
            attributes: [[Sequelize.fn('AVG', Sequelize.col('calificacion_final')), 'promedioCalificacionFinal']],
            where: {
              agente_dni: dni,
              [Op.and]: [
                Sequelize.where(
                  Sequelize.fn('STR_TO_DATE', Sequelize.col('fecha_llamada'), '%d/%m/%Y'),
                  {
                    [Op.gt]: parsedDate
                  }
                )
  ...
  const addFeedbackData = catchAsync(async (req, res, next) => {
    ...
    if (!ficha) {
      ...
      return next(new AppError(`Evaluación con id ${idevaluacion} no encontrado`, 404))
    }
