...
diarize_model = whisperx.DiarizationPipeline(
    use_auth_token="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",
    device=DEVICE
)

def transcribir_audio(ruta_audio: str) -> dict:
    logger.info(f"📝 Transcribiendo: {ruta_audio}")
    return model.transcribe(ruta_audio, language="es")

def alinear_segmentos(result: dict, ruta_audio: str) -> dict:
    logger.info(f"📏 Alineando segmentos para: {ruta_audio}")
    return whisperx.align(result["segments"], model_a, metadata, ruta_audio, device=DEVICE)

def obtener_diarizacion(ruta_audio: str) -> list:
    logger.info(f"🧑‍🤝‍🧑 Obteniendo diarización: {ruta_audio}")
