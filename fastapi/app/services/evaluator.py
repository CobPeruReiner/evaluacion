...
from app.services.evaluaciones.Scotiabank.scotiabank_evaluator import evaluar_fase_scotiabank
...
def evaluar_llamada(transcripcion: list, id_cartera: str, tipificaciones: dict = None) -> dict:
    ...
    if id_cartera == "70": # Asumiendo que "70" es el ID de cartera para Scotiabank
        logger.info(f"⚙️ Iniciando evaluación específica para Scotiabank (ID Cartera {id_cartera})...")
        resultados["scotiabank_evaluacion"] = evaluar_fase_scotiabank(transcripcion, id_cartera, tipificaciones)
    else:
        # Mantienes tu lógica actual para otras carteras
        logger.info(f"⚙️ Iniciando evaluación general para cartera {id_cartera} (Apertura, Indagación)...")
        resultados["apertura"] = evaluar_apertura(segmentos_asesor, id_cartera)
        resultados["indagacion"] = evaluar_indagacion(segmentos_asesor, id_cartera, tipificaciones)
