...
def seleccionar_accion_saludo(texto: str, acciones: list, id_cartera: str) -> dict:
    ...
    saludos_regular = [
        "buenos dias", "buenas tardes", "buenas noches", "muy buenos dias",
        "muy buenas tardes", "muy buenas noches", "buenas", "buen dia"
    ]
...
def seleccionar_accion_contacto(texto: str, acciones: list, id_cartera: str) -> dict:
    ...
    frases_regular = [
        "me comunico con", "me comunica con", "me estoy comunicando con", "con el sr", "con la sra",
        "con el senor", "con la senora", "se encuentra", "te saluda", "te saludo", "me podria comunicar con",
        "es usted el senor", "es usted la senora", "es usted la senorita", "nos comunicamos con la senora", "nos comunicamos con el senor",
        "nos comunicamos con la senorita", "es la senora", "es el senor", "el senor", "la senora"
    ]
    ...
    for f in frases:
        ...
        nombre_potencial = [
            p for p in palabras
            if p not in palabras_invalidas
            and p not in palabras_dudosas
            and len(p) >= 3
        ]
...
def seleccionar_accion_identificacion(texto: str, acciones: list) -> dict:
    ...
    tiene_presentacion = any(p in texto for p in [
        "le saluda", "soy", "habla", "que llamo", "les saluda", "mi nombre es",
        "le saludo", "les saludo", "de parte de", "te saluda",
        "le acaba de saludar", "le acabo de saludar", "le hace presente que nos hemos comunicado"
    ])

    tiene_empresa = any(p in texto for p in [
        "de cobranzas", "represento a", "por encargo de", "de parte de"
    ])
