...
from app.services.indagacion_acciones import (
    ...
    try:
        ...
        for criterio in criterios:
            ...
            if "SITUACIÓN" in nombre:
                accion = seleccionar_accion_info_producto(texto, acciones, id_cartera, tipificaciones)
            elif "INDAGAR MOTIVO" in nombre:
                accion = seleccionar_accion_indagar_pago(texto, acciones, id_cartera, tipificaciones)
            elif nombre == "ASESORAR":
                accion = seleccionar_accion_asesorar(texto, acciones, id_cartera)
            else:
                continue

            accion["PESO_ACCION_CRITERIO"] = float(accion.get("PESO_ACCION_CRITERIO", 0))
