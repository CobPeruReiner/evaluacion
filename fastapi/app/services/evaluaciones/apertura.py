...
from app.services.apertura_acciones import (
...
def evaluar_apertura(segmentos: list, id_cartera: str) -> dict:
    ...
    try:
        ...
        for criterio in criterios:
            ...
            if nombre == "SALUDO":
                accion_detectada = seleccionar_accion_saludo(texto_asesor, acciones, id_cartera)
            elif nombre == "CONTACTAR CON LA PERSONA ADECUADA":
                accion_detectada = seleccionar_accion_contacto(texto_asesor, acciones, id_cartera)
            elif nombre == "IDENTIFICACIÓN DEL GESTOR":
                accion_detectada = seleccionar_accion_identificacion(texto_asesor, acciones)
            else:
                continue

            if accion_detectada:
                accion_detectada["PESO_ACCION_CRITERIO"] = float(accion_detectada.get("PESO_ACCION_CRITERIO", 0))
                resultado_criterios[criterio["NOMBRE_CRITERIO"]] = accion_detectada

                if accion_detectada["NOMBRE_ACCION_CRITERIO"].strip().upper() == "SÍ CUMPLE":
                    cumplidos += 1

        porcentaje = (cumplidos / len(resultado_criterios)) * 100 if resultado_criterios else 0
